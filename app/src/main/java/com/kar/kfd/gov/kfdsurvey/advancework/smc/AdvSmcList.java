...
public class AdvSmcList extends BaseActivity implements View.OnClickListener, SMCAdapter.ViewCallBack {

    public static final String TAG = AdvSmcList.class.getSimpleName();
    ...
    private void callSMCWorks() {
        ...
        Cursor cursor = db.getAdvSmcWorks(formId);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.saveSMC) {
            ...
            for (SMCModel sml : list) {
                ...
                db.updateTableWithId(Database.TABLE_ADV_SMC_LIST, Database.SMC_ID, cvsmcWorks);
            }
    ...
    void calculateHighest() {
        ...
        if (pref.getString(Database.SMC_STATUS, "0").equals("0")) {
            ...
            db.updateTableWithId(Database.TABLE_ADV_SMC_MASTER, Database.FORM_ID, cv);
        }
        Cursor cursorParent = db.getAdvsmcHighest(formId);
        Cursor cursorSMCHighchild = db.getAdvSMCHighestWithFormID(formId);
        if (cursorParent != null && cursorParent.moveToFirst()) {
            ...
            if (cursorSMCHighchild != null && cursorSMCHighchild.moveToFirst()) {
                ...
                String smcpath = this.getExternalFilesDir(null) + "/Photo/" + SmcAdvanceWork.folderName + File.separator;
                for (Integer i : smcIds) {
                    File file = new File(smcpath + i);
                    deleteDir(file);
                }

                db.deleteSMCHigh(Database.TABLE_ADV_SMC_HIGHEST, Database.FORM_ID, formId);
