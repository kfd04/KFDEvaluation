...
public class AddSpecies extends HBaseFormActivity {

    public static final String folderName = Constants.FORMTYPE_ADD_SPECIES;
    public static final String SPECIES_OTHER = "species_other";
    public static final String OTHERS_IF_ANY_SPECIFY = "Others";
    public static final String SPECIES_PREF = "AddSpecies";
    public static int screenWidthInPixels = 0;
    public static DisplayMetrics metrics;
    private SweetAlertDialog dialog;
    private AddSpecies mSurvey = this;
    private Database db;
    ...
    protected HRootElement createRootElement() {
        ...
        String modelId = pref.getString("model_id", "1");
        ...
        /*HPickerElement mainSpeciesPlanted = new HPickerElement(Database
                .MAIN_SPECIES_PLANTED, "8.Species planted", "Select species names", true,
                "others|" + db.getNamesOfSpeciesNew(modelId), store);*/

        HTextEntryElement samplePlotNo = new HTextEntryElement(Database.SAMPLE_PLOT_ID, "1.Sample species No.", "Enter sample species no", true, store);
        samplePlotNo.setNotEditable();

        if (Integer.parseInt(prefSpecies.getString(Database.SAMPLE_PLOT_ID, "0")) == 0) {
            samplePlotNo.setValue(String.valueOf(db.getNumberOfSampleSpecies(Integer.parseInt(prefSpecies.getString(Database.FORM_ID, "0"))) + 1));
        } else {
            // samplePlotNo.setValue(String.valueOf(db.getSpeciesNumberForFormId(Integer.parseInt(prefSpecies.getString(Database.FORM_ID,"0")),Integer.parseInt(prefSpecies.getString(Database.SAMPLE_PLOT_ID,"0")))));
        }
        basicInfoSection.addEl(samplePlotNo);

        Log.e("adscasc",""+modelId);
        HPickerElement mainSpeciesPlanted = new HPickerElement(Database
                .MAIN_SPECIES_PLANTED, Database.SPECIES_ID, "2.Species planted", "Select species names", true,
                0, db.getNamesofSpeciesWithId(modelId), store);
        ...
        HPickerElement pbviseSize = new HPickerElement(Database.SPECIES_SIZE, "3.Size of Seedlings" +
                " planted", "Select an option", true, "4 X 6|5 X 8|" +
                "6 X 9|8 X 12|10 X 16|14 X 20", store);
        basicInfoSection.addEl(pbviseSize);

        HNumericElement noOfSeedlings = new HNumericElement(Database.TOTAL_SPECIES_COUNT, "4.No of Seedlings", "Enter Number of seedlings", false, store);
        basicInfoSection.addEl(noOfSeedlings);

       /* HNumericElement plantationDensity = new HNumericElement(Database.PLANTING_DENSITY_HA,
                "10a.Planting Density as per field records(per ha ) ", "Enter the planting " +
                "density ( no of seedlings per ha ) ", false, store);
        basicInfoSection.addEl(plantationDensity);*/
    ...
    private void submitSpeciesDetails() {
        ...
        customDialogLayout.findViewById(R.id.alert_submit).setOnClickListener(v -> {
            Map<String, ArrayList<String>> tableMetadata = getTableMetaData(Database.TABLE_ADD_SPECIES, db);
            ...
            if (Integer.parseInt(prefSpecies.getString(Database.INVENTORY_ID, "0")) == 0) {
                db.insertIntoSpeciesInventory(cv);
                cv.put(Database.PART_TYPE, "Seedling");
                db.insertIntoSamplePlotSpecies(cv);
            } else {
                cv.put(Database.INVENTORY_ID, prefSpecies.getString(Database.INVENTORY_ID, "0"));
                db.updateTableWithId(Database.TABLE_ADD_SPECIES, Database.INVENTORY_ID, cv);
                db.updateTableWithId(Database.TABLE_SAMPLEPLOT_SPECIES, Database.INVENTORY_ID, cv);
            }
    ...
    public void onBackPressed() {
        ...
        if (count == 0) {
            ...
            if (pref1.getString("formStatus", "0").equals("0")) {
                Log.d("FormStatus", pref1.getString("formStatus", "0"));
//                showSaveFormDataAlert();
    ...
    public void showSaveFormDataAlert() {
        ...
        alertDialogBuilder.setPositiveButton("yes",
                (arg0, arg1) -> submitSpeciesDetails());
