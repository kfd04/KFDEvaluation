 ...
 * Modified by Sarath
 */
public class SmcAdvanceWork extends HBaseFormActivity {

    public static final String ADV_SMC_HIGHEST = "AdvSmcHighest";
    ...
    public static final String SMC_ADVANCE_WORK = "SmcAdvanceWork";
    private static final String TAG = SmcAdvanceWork.class.getSimpleName();
    int formId;
    private SmcAdvanceWork mSurvey = this;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbarTitle.setText(R.string.actionbar_title_smc_sampling);
    ...
    protected HRootElement createRootElement() {
        ...
        SharedPreferences preferences = this.getApplicationContext().getSharedPreferences(PlantationSamplingAdvanceWork.ADVANCE_WORK_SURVEY, Context.MODE_PRIVATE);
        try {
            formId = Integer.parseInt(preferences.getString(Database.FORM_ID, "0"));
        } catch (Exception e) {
            e.printStackTrace();
        }
        Cursor cursor = database.getAdvSmcWorks(String.valueOf(formId));
        ...
        final SharedPreferences pref = this.getApplicationContext().getSharedPreferences(SMC_ADVANCE_WORK, Context.MODE_PRIVATE);
       ...
       /* if(pref.getString(SMC_APPLICABLE,"").equals("")){
        ...
        HPickerElement isApplicable = new HPickerElement(SMC_APPLICABLE, "Are there any SMC works " +
                "taken up in this area?", "Select an option", true, "Yes|No", store);
        ...
        listWorks.setOnClick(v -> {
            Intent i = new Intent(mSurvey.getApplicationContext(), AdvSmcList.class);
        ...
        SmcWork1.setOnClick(v -> {
            ...
            Cursor cursor1 = db.getAdvSmcNumber1(formId);
            if (cursor1 != null && cursor1.moveToFirst()) {
                SharedPreferences pref1 = getSharedPreferences(ADV_SMC_HIGHEST, Context.MODE_PRIVATE);
        ...
        SmcWork2.setOnClick(v -> {
            ...
            Database db = new Database(mSurvey.getApplicationContext());
            Cursor cursor12 = db.getAdvSmcNumber2(formId);
            if (cursor12 != null && cursor12.moveToFirst()) {
                SharedPreferences pref12 = getSharedPreferences(ADV_SMC_HIGHEST, Context.MODE_PRIVATE);
                SharedPreferences.Editor editor;
       ...
       /* HPickerElement expenditurePerNorms = new HPickerElement(Database.SMC_WORK_EXPENDITURE_AS_PER_NORM, "2.Whether the SMC expenditure is as per the norms prescribed ( 15% of total planting cost ) ?", "Select an option", true, "Yes|No", store);
        ...
        HPickerElement builtInPlantation = new HPickerElement(Database.ALL_SMC_STRUCTURES_BUILT_IN_FARM, "2.Whether all the SMC structures were built within boundary?", "Select an option", true, "Yes|No", store);
        ...
        HPickerElement plantationTreated = new HPickerElement(Database.SMC_PLANTATION_AREA_TREATED_COMPLETELY, "3.Whether the Advance work area was treated completely?", "Select an option", true, "Yes|No", store);
        generalObservations.addEl(plantationTreated);

        HPickerElement watershedPattern = new HPickerElement(Database.SMC_TREATMENT_FOLLOWED_WATERSHED_PATTERN, "4.Whether SMC treatment followed across the slope?", "Select an option", true, "Yes|No", store);
       ...
       /* HPickerElement dimensionsAppropriate = new HPickerElement(Database.SMC_WORK_DIMENSION_OK_FOR_LOCATION,"7.Are the dimensions of SMC structures appropriate to the locations?","Select an option",true,"Yes|No",store);
        generalObservations.addEl(dimensionsAppropriate);*/

       /* HTextAreaEntryElement dimensionsInappropriateReasons = new HTextAreaEntryElement(Database.SMC_WORK_DIMENSION_NOTOK_FOR_LOCATION_DETAILS,"Details","Enter details of inappropriate dimensions",true,store);
        generalObservations.addEl(dimensionsInappropriateReasons);*/
        //       dimensionsAppropriate.addNegElement(dimensionsInappropriateReasons);
       ...
       /* HPickerElement damageToTrees = new HPickerElement(Database.ANY_DAMAGE_CAUSED_TO_TREE_ROOTS_BY_SMC_WORK,"10.Was any damage done to local trees or root zone by the SMC works?","Select an option",true,"Yes|No",store);
        ...
        HPickerElement othersmc = new HPickerElement(Database.OTHER_SMC_WORKS, "1.Is there any other SMC works noticed in Advance Work area?", "Select an option", true, "Yes|No", store);
        ...
        othersmc.addValueChangedListener(el -> {
            ...
            if (pickerItem.getValue().equalsIgnoreCase("yes")) {
                ...
                i.putExtra("id", Integer.parseInt(pref.getString(Database.FORM_ID, "0")));
                i.putExtra("List-type", Constants.ADV_OTHER_SMC_WORKS);
                i.putExtra("formStatus", formStatus);
                startActivity(i);
            }
    ...
    public void onBackPressed() {
        if (!checkFormData()) {
            ...
            SharedPreferences pref = getSharedPreferences(SMC_ADVANCE_WORK, Context.MODE_PRIVATE);
    ...
    void calculateHighest() {
        ...
        database.updateTableWithFormId(Database.TABLE_ADV_SMC_MASTER, formId, cv);
        pref.edit().putString(Database.SMC_STATUS, "1").apply();

        Cursor cursorParent = database.getAdvsmcHighest(formId);
        Cursor cursorSMCHighchild = database.getAdvSMCHighestWithFormID(formId);
        if (cursorParent != null && cursorParent.moveToFirst()) {

            if (cursorSMCHighchild != null && cursorSMCHighchild.moveToFirst()) {


                database.deleteSMCHigh(Database.TABLE_ADV_SMC_HIGHEST, Database.FORM_ID, formId);
                do {
                    ...
                    database.insertIntoAdvInspectedSmcWorkDetails(cvSMCHigh);
