...
public class AddOtherSMC extends HBaseFormActivity {

    public static final String ADD_OTHER_SMC = "add_other_smc";
    ...
    protected HRootElement createRootElement() {
        ...
        SharedPreferences preferences = this.getApplicationContext().getSharedPreferences(ADD_OTHER_SMC, Context.MODE_PRIVATE);
    ...
    private void submitOtherSmcWorks() {
        SharedPreferences pref = this.getApplicationContext().getSharedPreferences(ADD_OTHER_SMC, Context.MODE_PRIVATE);
        Database db = new Database(this.getApplicationContext());
        Map<String, ArrayList<String>> tableMetadata = getTableMetaData(Database.TABLE_OTHER_SMC_LIST, db);
        ...
        if (Integer.parseInt(pref.getString(Database.OTHER_SMC_ID, "0")) == 0) {
            ...
            db.insertOtherSMCList(cv);
        } else {
            cv.put(Database.FORM_FILLED_STATUS, formFilledStatus);
            cv.put(Database.OTHER_SMC_ID, Integer.parseInt(pref.getString(Database.OTHER_SMC_ID, "0")));
            db.updateTableWithId(Database.TABLE_OTHER_SMC_LIST, Database.OTHER_SMC_ID, cv);
        }
